...
| Config | [`config.md`](config.md) | `internal/config` | Defines, validates (including Discord field bounds), migrates, reads settled by default with explicit unsettled opt-outs, watches with ordered reload results plus partial-snapshot protection and an `enabled`-specific loosening guard for non-atomic writes, initializes, fail-closes invalid existing files, and saves user configuration. |
