...
| Config | [`config.md`](config.md) | `internal/config` | Defines, validates (including Discord field bounds), migrates, watches with ordered reload results and full-budget provisional-snapshot protection for non-atomic writes, initializes, fail-closes invalid existing files, loads, and saves user configuration. |
