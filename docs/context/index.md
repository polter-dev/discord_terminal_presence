...
| Config | [`config.md`](config.md) | `internal/config` | Defines, validates (including Discord field bounds), migrates, performs deterministically tested bounded settled reads with a distinguishable destructive-load busy error and horizon-safe writeback defaults plus explicit read-only/unsettled opt-outs, watches with ordered reload results plus partial-snapshot protection and an `enabled`-specific loosening guard for non-atomic writes, initializes, fail-closes invalid existing files, and saves user configuration. |
