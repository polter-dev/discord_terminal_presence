...
| Config | [`config.md`](config.md) | `internal/config` | Defines, validates (including Discord field bounds), migrates, watches with ordered reload results plus partial-snapshot protection and an `enabled`-specific loosening guard for non-atomic writes (reload only; see the module doc for known gaps), initializes, fail-closes invalid existing files, loads, and saves user configuration. |
