...
| Config | [`config.md`](config.md) | `internal/config` | Defines, validates (including Discord field bounds), migrates, watches with ordered reload results, settle-checked non-atomic-write safety, and atomic malformed-reload coverage, initializes, fail-closes invalid existing files, loads, and saves user configuration. |
...
| Presence | [`presence.md`](presence.md) | `internal/presence` | Maps detections to defensively bounded activity, sanitizes Discord-facing text at the wire payload boundary, reports optional-field omissions for caller-gated diagnostics, and classifies payload versus transport IPC failures. |
