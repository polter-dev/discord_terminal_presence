...
| Config | [`config.md`](config.md) | `internal/config` | Defines, validates (including Discord field bounds), migrates, watches with ordered reload results and non-atomic editor coverage, initializes, fail-closes invalid existing files, loads, and saves user configuration. |
