...
func runAutomaticUpdateWithStatePathForPlatform(ctx context.Context, cfg config.Config, current string, checker automaticUpdateChecker, runner updatepkg.CommandRunner, statePath, goos string) {
	...
	installerPID := os.Getpid()
	installerStartTime, _ := processStartTime(installerPID)
	if err := updatepkg.RecordAutomaticUpdateAttemptForProcess(statePath, result.Latest, time.Now(), nil, installerPID, installerStartTime); err != nil {
		debugf("automatic update success could not be recorded: %v", err)
	}
	// The success and installer identity are recorded above so `termp status`
	// can show the notice only while this exact daemon keeps running the old
	// code. The normal log is the immediate surface when the daemon has no
	// attached terminal (issue #584).
	log.Printf("Automatic update installed %s. The background daemon is still running the previous version. Run \"termp stop\" then \"termp start\" to finish updating.", result.Latest)
}
