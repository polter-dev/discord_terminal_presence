...
func writeSucceededAttempt(t *testing.T, target string) string {
	...
	if err := updatepkg.RecordAutomaticUpdateAttempt(statePath, target, time.Now(), nil); err != nil {
		t.Fatalf("RecordAutomaticUpdateAttempt() error = %v", err)
	}
	return statePath
}

// The automatic path runs inside the daemon and had no user to print to, so its
